	...
	int*& productCoef, int*& productExpon, int& productSize)
{
	...
	if (!productCoef)
		productCoef = new int[arraySize]();

	if (!productExpon)
		productExpon = new int[arraySize]();

	if (!isZero(multiplicandSize) && !isZero(multiplierSize))
		for (size_t i = 0; i < multiplierSize; ++i)
		{
			for (size_t j = 0; j < multiplicandSize; ++j)
			{
