...
void Polynomial::addition( Polynomial adder )
{
   ...
   else if (!adder.size)
   {
       for (; i < this->size; ++i, ++k)
       {
           sum.terms[k].coef = this->terms[i].coef;
           sum.terms[k].expon = this->terms[i].expon;
       }
   }
   else
   {
       while (true)
       {
           if (this->terms[i].expon > adder.terms[j].expon)
           {
               sum.terms[k].coef = this->terms[i].coef;
               sum.terms[k].expon = this->terms[i].expon;
               ++i, ++k;
           }
           else if (this->terms[i].expon == adder.terms[j].expon)
           {
               if (this->terms[i].coef + adder.terms[j].coef == 0)
                   ++i, ++j;
               else
               {
                   sum.terms[k].coef = this->terms[i].coef + adder.terms[j].coef;
                   sum.terms[k].expon = this->terms[i].expon;
                   ++i, ++j, ++k;
               }
           }
...
void Polynomial::multiplication( Polynomial &multiplier, Polynomial &product )
{
   ...
   buffer.terms = new Term[this->size]();

   //if (!product.terms)
   //    product.terms = new Term[arraySize];
   ...
   if( size != 0 && multiplier.size != 0 )
   {
       for (int i = 0; i < multiplier.size; ++i)
       {
           ...
           {
               buffer.terms[j].coef = this->terms[j].coef * multiplier.terms[i].coef;
               buffer.terms[j].expon = this->terms[j].expon + multiplier.terms[i].expon;
           }
...
void Polynomial::division( Polynomial &divisor, Polynomial &quotient, Polynomial &remainder )
{
   ...
   buffer.terms = new Term[divisor.size]();
   ...
   while (remainder.size > 0 && remainder.terms[0].expon >= divisor.terms[0].expon)
   {
       ...
       //cout << "temp:    ";
       //output(tempCoef, tempExpon, quotientSize + 1);
