                  ...
                  int numDicWords, char ( *&result )[ 21 ], int &numResult )
{
   ...
   for( int i = 0; i < numWords; i++ )
      for( int j = 0; j < length; j++ )
      {
          ...
          memmove(buffer + j, buffer + j + 1, length - j);

          if (!strcmp(wordToCheck, buffer) && strcmp(result[numResult - 1], words[i]))    //words[i].compare(result[numResult - 1]) != 0)
          {
              strcpy_s(result[numResult], words[i]);
              ++numResult;
          }
                  ...
                  int numDicWords, char ( *&result )[ 21 ], int &numResult )
{
   ...
   for( int i = 0; i < numWords; i++ )
      ...
      {
          strcpy_s(buffer, wordToCheck);
          memmove(buffer + j, buffer + j + 1, length - j);
          buffer[length] = '\0';

          if (!strcmp(words[i], buffer) && strcmp(result[numResult - 1], words[i]))
          {
              strcpy_s(result[numResult], words[i]);
              ++numResult;
          }
