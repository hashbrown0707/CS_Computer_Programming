...
public:
	...
	vector& operator=(const vector& right)
	{
		if (this != &right) // avoid self-assignment
		{
			...
			else if (right.size() > capacity() && right.size() <= capacity() * 3 / 2) 
			{
				pointer tempPtr;

				tempPtr = myData.myFirst;
				myData.myFirst = new value_type[capacity() * 3 / 2];
				myData.myLast = myData.myFirst + right.size();
				myData.myEnd = myData.myFirst + (myData.myEnd - tempPtr) * 3 / 2;
