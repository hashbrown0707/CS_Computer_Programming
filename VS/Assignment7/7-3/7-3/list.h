...
public:
	...
	list& operator=(const list& right)
	{
		...
		if (right.size() < this->size())
		{
			...
			myData.myHead->prev = it1.ptr->prev;
			it1.ptr->prev->next = myData.myHead;
		}
		...
		else
		{
			...
			for (; it2 != right.cend(); it2++)
			{
				...
				newNode->myVal = *it2;
				newNode->next = it1.ptr;
				newNode->prev = it1.ptr->prev;
				it1.ptr->prev->next = newNode;
				it1.ptr->next->prev->prev = newNode;
			}

			/*nodePtr newNode = new node;
			newNode->myVal = val;
			newNode->next = where.ptr->next->prev;
			newNode->prev = where.ptr->prev;
			where.ptr->prev->next = newNode;
			where.ptr->next->prev->prev = newNode;
			myData.mySize++;*/
	...
	{
		if (myData.mySize == 0 || where == myData.myHead)
			return myData.myHead->next;

		iterator returnIt = where.ptr->next;
...
bool operator==(const list< Ty >& left, const list< Ty >& right)
{
	...
	typename  list<Ty>::const_iterator it1 = left.cbegin();
	typename  list<Ty>::const_iterator  it2 = right.cbegin();
