...
public:
	...
	const MyDeque* myCont; // keep a pointer to deque  (myContainer)
...
#pragma region Operator&Position
...
private:
	...
	void doubleMapSize()
	{
		if (myData.mapSize > 0)
		{
			...
			for (size_t i = myData.myOff, j = i; j != (myData.myOff + myData.mySize) % (4 * myData.mapSize); i++, j++)  //i => map index, j => m index
			{
				i = i >= 4 * originalMapSize ? 0 : i;
				j = j >= 4 * myData.mapSize ? 0 : j;
				m[getBlock(j)][j % 4] = myData.map[getBlock(i)][i % 4];
			}
