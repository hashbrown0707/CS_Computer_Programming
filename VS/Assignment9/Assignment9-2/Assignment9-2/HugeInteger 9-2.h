...
HugeInteger< T > HugeInteger< T >::operator-(HugeInteger< T >& op2)
{
	...
	for (; it4 != difference.integer.crend() && *it4 == 0; it4 = difference.integer.crbegin())
...
HugeInteger< T > HugeInteger< T >::operator*(HugeInteger< T >& op2)
{
	...
	HugeInteger<T> temp(productSize);

	typename T::iterator it1 = integer.begin();

	for (size_t i = 0; it1 != integer.end(); ++i, ++it1)
	{
		for (size_t j = *it1; j > 0; --j)
			temp = temp + op2;

		for (size_t k = 0; k < i; ++k)
			temp.integer.insert(temp.integer.cbegin(), 0);
	...
	for (; it2 != product.integer.crend() && *it2 == 0; it2 = product.integer.crbegin())
