...
void addition(int* addend, int* adder, int& addendDegree, int adderDegree)
{
	for (size_t i = 0; i <= adderDegree; ++i)
		addend[i] += adder[i];

	for (size_t i = addendDegree; i > 0; --i)
		if (addend[i] == 0)
			addendDegree--;
		else
			break;
...
void subtraction(int* minuend, int* subtrahend, int& minuendDegree, int subtrahendDegree)
{
	for (size_t i = 0; i <= subtrahendDegree; ++i)
...
void multiplication(int* multiplicand, int* multiplier, int* product,
	int multiplicandDegree, int multiplierDegree, int productDegree)
{
	for (size_t i = 0; i <= multiplicandDegree; ++i)
		for (size_t j = 0; j <= multiplierDegree; ++j)
...
void division(int* dividend, int* divisor, int* quotient, int* remainder,
	int dividendDegree, int divisorDegree, int quotientDegree, int& remainderDegree)
{
	for (size_t i = 0; i <= remainderDegree; ++i)
	...
	while (true)
	{
		...
		int monominalDegree = remainderDegree - divisorDegree;
		int* monominal = new int[monominalDegree + 1]();

		quotient[monominalDegree] = remainder[remainderDegree] / divisor[divisorDegree];
		monominal[monominalDegree] = quotient[monominalDegree];

		multiplication(monominal, divisor, buffer, monominalDegree, divisorDegree, bufferDegree);
		subtraction(remainder, buffer, remainderDegree, bufferDegree);

		delete[] buffer;
		delete[] monominal;

		if ((remainderDegree < divisorDegree) || isZero(remainder, remainderDegree))   // || ((remainderDegree == divisorDegree) && (remainder[remainderDegree] < divisor[divisorDegree]))
			break;
	}
}