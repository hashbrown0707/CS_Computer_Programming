	...
	int dividendSize, int divisorSize, int &quotientSize, int &remainderSize )
{
	...
	int buffer[ arraySize + 10 ] = { 0 };
	int bufferSize = dividendSize;
	int n = dividendSize - divisorSize;
	quotientSize = n;

	for( int i = 0, j = n; i < divisorSize; ++i, ++j )
		buffer[ j ] = divisor[ i ];
	...
	int anotherBuffer[ arraySize ] = { 0 };
	int anotherBufferSize = 1;
	...
	for( int i = dividendSize - divisorSize; i >= 0; --i, --j )
	{
		...
		if( a <= b )
		{
			...
			for( int i = 0; i < anotherBufferSize; ++i )
				anotherBuffer[ i ] = 0;
			multiplication( divisor, quotient[ i ], anotherBuffer, divisorSize, i, anotherBufferSize );

			while( less(remainder, anotherBuffer, remainderSize, anotherBufferSize) )
			{
				--quotient[ i ];
				for( int i = 0; i < anotherBufferSize; ++i )
					anotherBuffer[ i ] = 0;
			...
			subtraAssign( remainder, anotherBuffer, remainderSize, anotherBufferSize );
		}
	}
...
void multiplication( int multiplicand[], int multiplier, int product[],
	int multiplicandSize, int multiplierPos, int &productSize )
{
	int multiplierSize = 0, temp = multiplier;
	...
	productSize = multiplicandSize + multiplierPos + multiplierSize;
